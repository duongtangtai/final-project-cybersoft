...
export class ProjectComponent implements OnInit {
  ...
  initDisplayedColumns() {
    //init display columns based on USER ROLES
  ...
  getAllProject() {
    ...
    if (AppSettings.USER_ROLES.includes(AppSettings.ROLE_MANAGER)) {
      ...
      this.projectService.getProjectsWithInfo().subscribe((result) => {
        ...
        this.projects = result.filter(
          (project: any) => project.leader.username == leaderUsername
        );
        this.pagingAndSorting();
      });
    }
  ...
  getAllProjectWithStatus(status: string) {
    ...
    if (AppSettings.USER_ROLES.includes(AppSettings.ROLE_MANAGER)) {
      ...
      this.projectService.getProjectsWithInfo().subscribe((content) => {
        ...
        this.projects = content.filter(
          (project: any) =>
            project.leader.username == leaderUsername &&
  ...
  statusProjectAfterClosePopup() {
    return this.statusProject
      ? this.getAllProjectWithStatus(this.statusProject)
      : this.getAllProject();
  }
}
